...
const selectedButton = ({path, name}) =>
  h('li',
    h('a.selected', {props: {href: path}}, name)
  );

const notSelectedButton = ({path, name}) =>
  h('li',
    h('a', {props: {href: path}}, name)
  );
...
export const completedTodosButton = pairOfButtons({path: '#/completed', name: 'Completed'});

export const activeTodosButton = pairOfButtons({path: '#/active', name: 'Active'});

export const makeBinding = ({selected, buttons: {allTodosButton, completedTodosButton, activeTodosButton}}) => () => ({
